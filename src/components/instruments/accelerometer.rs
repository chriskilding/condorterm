...
pub fn Accelerometer(props: &AccelerometerProps) -> impl Into<AnyElement<'static>> {
    element! {
        Box(flex_direction: FlexDirection::Column) {
            ...
            Box(border_style: INSTRUMENT_BORDER_STYLE, border_color: INSTRUMENT_BORDER_COLOR, flex_direction: FlexDirection::Column, width: BOX_WIDTH, padding_top: 1, padding_left: 1, padding_right: 1) {
                Box(background_color: Color::Grey, height: 1) {
                    Box(width: 9, border_style: BorderStyle::None) {}
                    Box(width: 2, border_style: BorderStyle::None, background_color: bar_color(props.value, &props.scale)) {}
                }
